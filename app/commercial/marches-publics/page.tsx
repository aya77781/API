...
import { ExternalLink, FileText, X, Ban, Check, Loader2, EyeOff, RefreshCw, AlertCircle } from 'lucide-react'
...
const STATUT_OPTIONS: { key: Statut; label: string; badge: string }[] = [
  { key: 'nouvelle',       label: 'Nouvelle',       badge: 'bg-blue-100 text-blue-700' },
  { key: 'en_etude',       label: 'En étude',       badge: 'bg-orange-100 text-orange-700' },
  { key: 'interesse',      label: 'Intéressé',      badge: 'bg-violet-100 text-violet-700' },
  { key: 'dossier_depose', label: 'Dossier déposé', badge: 'bg-amber-100 text-amber-700' },
  { key: 'gagne',          label: 'Gagné',          badge: 'bg-emerald-100 text-emerald-700' },
  { key: 'perdu',          label: 'Perdu',          badge: 'bg-red-100 text-red-700' },
  { key: 'ignore',         label: 'Ignoré',         badge: 'bg-gray-100 text-gray-600' },
]
...
const FILTERS: { label: string; value: Statut | null }[] = [
  ...
  { label: 'Intéressées', value: 'interesse' },
  { label: 'Déposées',    value: 'dossier_depose' },
  { label: 'Gagnées',     value: 'gagne' },
  { label: 'Perdues',     value: 'perdu' },
  { label: 'Ignorées',    value: 'ignore' },
]
...
export default function MarchesPublicsPage() {
  ...
  async function handleRefresh() {
    ...
    const beforeIds = new Set(marches.map(m => m.id))
    try {
      ...
      const rows = await loadMarches()
      const added = rows.filter(r => !beforeIds.has(r.id)).length
      setRefreshMessage({
        type: 'success',
        text: added === 0
          ? 'Aucune nouvelle offre'
          : `${added} nouvelle${added !== 1 ? 's' : ''} offre${added !== 1 ? 's' : ''} ajoutée${added !== 1 ? 's' : ''}`,
      })
      setTimeout(() => setRefreshMessage(null), 5000)
    } catch {
      setRefreshMessage({ type: 'error', text: 'Erreur de connexion — réessayer' })
    } finally {
      setRefreshing(false)
    }
  }
  ...
  const nouvellesAujourdhui = useMemo(() => {
    ...
    return marches.filter(m => (m.date_publication ?? m.created_at.slice(0, 10)) === today).length
  }, [marches])

  const filtered = useMemo(() => {
    if (!filtre) return marches
...
function MarcheRow({
  marche, onSetStatut, onOpenNotes,
}: {
  marche: Marche
  ...
  return (
    <tr className={cn('hover:bg-gray-50 transition-colors', isIgnored && 'opacity-60')}>
      <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
        {marche.date_publication ? formatDateShort(marche.date_publication) : '—'}
      </td>
      ...
      <td className="px-4 py-3 text-right">
        {!isIgnored ? (
          <button
            onClick={() => onSetStatut(marche.id, 'ignore')}
          ...
          <button
            onClick={() => onSetStatut(marche.id, 'nouvelle')}
            title="Restaurer en nouvelle"
            className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors"
          >
