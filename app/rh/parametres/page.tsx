...
export default function RHParametresPage() {
  ...
  /* ── Completion % ── */
  const profilFields: (keyof ProfilData)[] = [
    'prenom', 'nom', 'date_naissance', 'lieu_naissance', 'nationalite',
    'numero_secu', 'statut_emploi',
    'adresse', 'code_postal', 'ville', 'telephone_perso', 'email_perso',
    'rib_iban', 'rib_bic',
    'contact_urgence_nom', 'contact_urgence_tel', 'contact_urgence_lien',
  ]
  const filledFields = profilFields.filter(f => profil[f]?.trim()).length
  const hasDoc = docs.length > 0
  const totalSteps = profilFields.length + 1
  const completedSteps = filledFields + (hasDoc ? 1 : 0)
      ...
      <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-4">
          ...
          <p className="text-xs text-gray-400 mt-2">
            {completionPct === 100
              ? 'Toutes les informations sont renseignees'
              : `${completedSteps}/${totalSteps} champs remplis -- completez votre profil pour atteindre 100%`}
          </p>
        </div>
