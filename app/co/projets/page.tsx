...
export default function COProjetsList() {
  ...
  return (
    <div>
      <TopBar
        title="Mes projets"
        subtitle={`${projets.length} dossier${projets.length !== 1 ? 's' : ''} assignés`}
      />
      <div className="p-6 space-y-3">
        {projets.length === 0 ? (
          ...
          projets.map(projet => <ProjetCard key={projet.id} projet={projet} roleBase="co" />)
        )}
      </div>
    </div>
  )
...
function ProjetCard({ projet, roleBase }: { projet: Projet; roleBase: string }) {
