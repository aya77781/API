...
export function CHOSidebar() {
  ...
  const navLinks = [
    { label: 'Tableau de bord', href: '/cho/dashboard',     icon: LayoutDashboard, badge: 0 },
    { label: 'Projets',         href: '/cho/projets',        icon: FolderOpen,      badge: 0 },
    { label: 'Climat Social',   href: '/cho/climat-social', icon: Heart,           badge: 0 },
    ...
    { label: 'Notes de frais',  href: '/cho/notes-frais',   icon: Receipt,         badge: 0 },
    ...
    { label: 'Paramètres',      href: '/cho/parametres',    icon: Settings,        badge: 0 },
  ]
      ...
      <div className={`border-t border-gray-100 ${collapsed ? 'px-2 py-3' : 'px-4 py-4 space-y-3'}`}>
        {profil && (
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'}`}>
            <div className="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center text-xs font-semibold text-pink-600 flex-shrink-0">
              {initiales}
            </div>
            {!collapsed && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">Chief Happiness Officer</p>
              </div>
            )}
          </div>
        )}
